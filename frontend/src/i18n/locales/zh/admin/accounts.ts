export default {
    accounts: {
      ...
      dataImportHint: '选择 Sub2API v1/v2 导出或转换 JSON，先预览校验与冲突，再确认写入。',
      dataImportWarning: '导入前会先预览，不会在选择文件后直接写入。',
      dataImportFile: '数据文件',
      ...
      dataImportSuccess: '导入完成：创建 {account_created}，更新 {account_updated}，跳过 {account_skipped}',
      dataImportCompletedWithErrors: '导入部分完成：创建 {account_created}，更新 {account_updated}，跳过 {account_skipped}，账号失败 {account_failed}，代理失败 {proxy_failed}',
      dataImportPreview: '预览导入',
      dataImportPreviewing: '正在预览...',
      dataImportPreviewNoWrite: '预览阶段只校验和匹配，不会写入账号、代理、文件夹或标签。',
      dataImportPreviewSummary: '检测到 {accounts} 个账号、{proxies} 个代理',
      dataImportVersion: 'Sub2API 数据格式 v{version}',
      dataImportConfirm: '确认导入',
      dataImportFinalWarning: '确认后才会写入。强身份冲突默认跳过；各账号可改为更新现有账号或另建新账号。',
      importStepFile: '选择文件',
      importStepConfigure: '预览与设置',
      importStepResult: '导入结果',
      importWillCreate: '将创建 {count}',
      importWillUpdate: '将更新 {count}',
      importWillSkip: '将跳过 {count}',
      importUniformSettings: '统一设置',
      importSettingsPriority: '设置优先级：逐账号覆盖 > 已启用的统一设置 > JSON 原值 > 系统默认值。',
      importItemOverrides: '逐账号覆盖',
      importActionCreate: '另建新账号',
      importActionUpdate: '更新现有账号',
      importActionSkip: '跳过',
      importStrongConflict: '发现 {count} 个强身份冲突',
      importInvalid: '校验失败',
      importProxyPreview: '代理预览（{count}）',
      importProxyReuse: '复用现有代理',
      importProxyCreate: '将新建',
      importProxyInvalid: '代理无效',
      importCreated: '已创建',
      importUpdated: '已更新',
      importSkipped: '已跳过',
      importFailed: '失败',
      importSessionReady: '已定位并选中本次成功写入的 {count} 个账号。',
      ...
      cindy: {
        ...
        deleteConfirm: '将永久删除 {count} 个余额不足的 Cindy 账号。此操作无法撤销。',
