...
export function useAccountSelectionMetadata(ids: Ref<number[]>, rows: Ref<AccountSelectionIdentity[]>) {
  ...
  watch([() => ids.value.join(','), rows], async () => {
    ...
    try {
      for (let offset = 0; offset < missing.length; offset += 100) {
        const page = await list(1, 100, { account_ids: missing.slice(offset, offset + 100).join(','), lite: '1', include_scheduler_score: '0' }, { signal })
