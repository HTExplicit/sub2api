...
export const createModelAllowlistState = (
  ...
  const savedModels = normalizeModels(config?.models ?? [])
  return {
    enabled: config?.enabled ?? false,
    savedModels,
    // The saved allowlist is editable immediately, even when candidates are
    // skipped in simple mode, still loading, or unavailable.
    items: savedModels.map(id => ({ id, selected: true })),
  }
}
...
export const setModelAllowlistCandidates = (
  ...
  const selectNewCandidates = state.items.length === 0 && state.savedModels.length === 0
  // Async discovery only supplements the current form. Never reconstruct its
  // selections: the user may already have toggled, reordered, or added models.
  for (const id of normalizeModels(candidates)) {
    if (currentKnown.has(id)) continue
    state.items.push({ id, selected: selectNewCandidates })
    currentKnown.add(id)
  }
}
...
export const buildModelAllowlistConfig = (
  ...
  models: state.items.filter(item => item.selected).map(item => item.id),
})
