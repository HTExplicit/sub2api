    ...
    <div v-else class="space-y-6 p-6">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-none bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-none px-2 py-1 text-xs font-black ring-1 ring-inset shadow-outline', statusClass]">
              {{ detail.status_code }}
            </span>
          </div>
        </div>
        ...
        <div class="rounded-none bg-gray-50 p-4 dark:bg-dark-900">
          ...
          <div class="mt-1">
            <span :class="['inline-flex items-center rounded-none px-2 py-1 text-xs font-black ring-1 ring-inset shadow-outline', upstreamStatusClass]">
        ...
        <div v-if="detail.api_key_prefix" class="rounded-none bg-gray-50 p-4 dark:bg-dark-900">
      ...
      <div v-if="rootCauseMessage" class="rounded-none bg-amber-50 p-6 dark:bg-amber-900/10">
      ...
      <div class="rounded-none bg-gray-50 p-6 dark:bg-dark-900">
        ...
        <div v-else class="mt-4 space-y-4">
          <div v-for="section in diagnosticPayloadSections" :key="section.key">
            ...
            <pre class="max-h-[520px] overflow-auto rounded-none border border-gray-200 bg-white p-4 text-xs text-gray-800 dark:border-dark-700 dark:bg-dark-800 dark:text-gray-100"><code>{{ prettyJSON(section.value) }}</code></pre>
          </div>
        </div>
      </div>

      <!-- Upstream errors list (only for request errors) -->
      <div v-if="showUpstreamList" class="rounded-none bg-gray-50 p-6 dark:bg-dark-900">
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              <div class="text-xs font-black text-gray-900 dark:text-white">
                #{{ idx + 1 }}
                <span v-if="ev.type" class="ml-2 rounded-none bg-gray-100 px-2 py-0.5 font-mono text-[10px] font-bold text-gray-700 dark:bg-dark-700 dark:text-gray-200">{{ ev.type }}</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="inline-flex items-center gap-1.5 rounded-none px-1.5 py-1 text-[10px] font-bold text-primary-700 hover:bg-primary-50 disabled:cursor-not-allowed disabled:opacity-60 dark:text-primary-200 dark:hover:bg-dark-700"
            ...
            <pre
              ...
              class="mt-3 max-h-[240px] overflow-auto rounded-none border border-gray-200 bg-gray-50 p-3 text-xs text-gray-800 dark:border-dark-700 dark:bg-dark-900 dark:text-gray-100"
