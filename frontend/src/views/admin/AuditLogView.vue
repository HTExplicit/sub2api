    ...
    >
      ...
      <div v-else-if="detail" class="space-y-5 py-2">
        ...
        <div class="rounded-none border border-gray-200 bg-gray-50/60 p-5 dark:border-dark-700 dark:bg-dark-900">
          ...
          <div class="mt-3 flex items-center gap-2 rounded-none bg-white px-3 py-2 ring-1 ring-gray-200 dark:bg-dark-800 dark:ring-dark-600">
            <span class="rounded-none bg-gray-100 px-1.5 py-0.5 font-mono text-[11px] font-bold text-gray-700 dark:bg-dark-700 dark:text-gray-200">
              {{ detail.method }}
            </span>
        ...
        <section>
          ...
          <div class="break-all rounded-none bg-gray-50 p-3 font-mono text-xs leading-relaxed text-gray-600 dark:bg-dark-900 dark:text-gray-400">
            {{ detail.user_agent || '—' }}
          </div>
        </section>
        ...
        <section v-if="detail.request_body">
          ...
          <pre class="max-h-72 overflow-auto rounded-none bg-gray-50 p-4 font-mono text-xs leading-relaxed text-gray-600 dark:bg-dark-900 dark:text-gray-400">{{ prettyBody(detail.request_body) }}</pre>
        </section>
        ...
        <section v-if="detail.extra && Object.keys(detail.extra).length">
          ...
          <pre class="max-h-48 overflow-auto rounded-none bg-gray-50 p-4 font-mono text-xs leading-relaxed text-gray-600 dark:bg-dark-900 dark:text-gray-400">{{ JSON.stringify(detail.extra, null, 2) }}</pre>
        </section>
      </div>
    </BaseDialog>
