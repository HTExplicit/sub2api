...
import {
  ...
  selectedModelAllowlistCount,
  setModelAllowlistCandidates,
  toggleModelAllowlistItem,
} from "../groupModelAllowlist";

describe("groupModelAllowlist", () => {
  it("initializes saved models as selected before any candidate request completes", () => {
    ...
    expect(selectedModelAllowlistCount(state)).toBe(2);
  });
  ...
  it("preserves explicitly unselected saved candidates when candidates refresh", () => {
    ...
    setModelAllowlistCandidates(state, ["gpt-5.5", "gpt-5.4"]);
    toggleModelAllowlistItem(state, "gpt-5.5");
    setModelAllowlistCandidates(state, ["gpt-5.4", "gpt-5.5", "new-candidate"]);

    expect(state.items).toEqual([
      { id: "gpt-5.5", selected: false },
      { id: "gpt-5.4", selected: false },
      { id: "new-candidate", selected: false },
    ]);
  });

  it("preserves saved models when a custom entry is added while candidates are pending", () => {
    const state = createModelAllowlistState({
      enabled: true,
      models: ["saved-model", "legacy-model"],
    });
    expect(addCustomModelAllowlistItem(state, "custom-model")).toBeNull();
    const expected = {
      enabled: true,
      models: ["saved-model", "legacy-model", "custom-model"],
    };
    expect(buildModelAllowlistConfig(state)).toEqual(expected);

    setModelAllowlistCandidates(state, ["legacy-model", "new-candidate", "custom-model"]);

    expect(buildModelAllowlistConfig(state)).toEqual(expected);
    expect(state.items.at(-1)).toEqual({ id: "new-candidate", selected: false });
  });

  it("supplements candidates without resetting user ordering or selection", () => {
    const state = createModelAllowlistState({
      enabled: true,
      models: ["first-model", "second-model"],
    });
    toggleModelAllowlistItem(state, "first-model");
    moveModelAllowlistItem(state, 1, 0);
    addCustomModelAllowlistItem(state, "custom-model");

    setModelAllowlistCandidates(state, ["first-model", "discovered-model", "second-model"]);

    expect(state.items).toEqual([
      { id: "second-model", selected: true },
      { id: "first-model", selected: false },
      { id: "custom-model", selected: true },
      { id: "discovered-model", selected: false },
    ]);
    expect(buildModelAllowlistConfig(state).models).toEqual(["second-model", "custom-model"]);
  });
