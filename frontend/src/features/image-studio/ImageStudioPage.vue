...
async function clearHistory(): Promise<void> {
  if (!await confirmAction(t('imageStudio.clearConfirm'))) return
  const ownerKey = historyOwnerKey.value
  if (!ownerKey) return
  try {
...
function downloadImage(record: DisplayHistoryRecord, image: DisplayHistoryImage): void {
  void downloadBlob(image.blob, `${record.model}-${record.createdAt}.${extensionForMimeType(image.mimeType)}`)
}
