...
vi.mock('@/features/cindy/api', async () => {
  const actual = await vi.importActual<typeof import('@/features/cindy/api')>('@/features/cindy/api')
  return {
    ...actual,
...
vi.mock('@/features/cindy/nativeState', async () => {
  const { computed, inject } = await import('vue')
  return { useCindyDraft: () => savedDraft, useCindyAdminScope: () => ({ available: inject(nativeAvailabilityKey, computed(() => true)) }) }
})
...
function render(available = ref(true)) {
  return mount(CindyBalanceProbePanel, {
    ...
    global: {
      provide: { [nativeAvailabilityKey as symbol]: computed(() => available.value) },
      stubs: {
        Icon: true,
        ConfirmDialog: {
          props: ['show'],
