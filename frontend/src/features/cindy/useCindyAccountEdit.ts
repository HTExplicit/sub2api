...
import type { AccountEditChangesV1, AccountEditDefinitionV1, AccountEditModeTarget, AccountEditTarget, ProviderEditRequestV1 } from '@/types/accountEdit'
import { cindyEditDefinition } from './accountForm'
...
type AccountEditContribution = { account_edit: AccountEditDefinitionV1 }
const nativeDefinition: AccountEditContribution = { account_edit: cindyEditDefinition }
const digest = (value: unknown): value is string => typeof value === 'string' && /^[a-f0-9]{64}$/.test(value)
const revision = (item: AccountEditContribution) => JSON.stringify(item.account_edit)
const sameOwner = (a: AccountEditContribution, b: AccountEditContribution) => a.account_edit.credential_profile === b.account_edit.credential_profile
...
export function useCindyAccountEdit(options: {
  active(): boolean
  ...
  const candidate = computed(() => required.value ? nativeDefinition : undefined)
  const matches = (context: ProviderAccountEditContext, _item: AccountEditContribution) =>
    'native' in context.profile && context.profile.native === true
  ...
  const reason = computed(() => draft.value?.reason || (available.value ? undefined : 'account_edit_unavailable'))
  ...
  function request(): ProviderEditRequestV1 | undefined {
    ...
    return {
      expected_state_sha256: selected.context.edit_state_sha256,
