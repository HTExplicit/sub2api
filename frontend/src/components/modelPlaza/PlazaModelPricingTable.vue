    ...
    <table class="w-full min-w-[1000px] table-auto border-collapse text-sm tabular-nums">
      ...
      <thead>
        ...
        >
          ...
          >
            <div class="border-b border-gray-200 pb-2 text-muted dark:border-dark-600">
              {{ t('modelPlaza.table.officialPrice') }}
              <span class="ml-1 normal-case font-normal text-muted">{{ t('modelPlaza.table.unitPerMillion') }}</span>
            </div>
          </th>
          <th
            rowspan="2"
        ...
        <tr
          class="border-b border-gray-200 text-left text-[11px] font-medium uppercase leading-4 tracking-wide text-muted dark:border-dark-700"
        >
        ...
        >
          ...
          <template v-if="billingMode(m) === BILLING_MODE_TOKEN">
            <td class="pz-cell px-3 py-2.5 align-middle font-mono font-semibold text-gray-900 dark:text-gray-50">
              <template v-if="tokenIntervals(m).length">
                ...
                >
                  <span class="mr-1 font-sans font-normal text-muted" :title="tierHint(m)">{{ tierLabel(iv) }}</span>
            ...
            <td class="pz-cell px-3 py-2.5 align-middle">
              <template v-if="hasTierCachePricing(tokenIntervals(m))">
                ...
                >
                  <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                    <span class="font-sans font-normal text-muted">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                    {{ paidPerMillion(iv.cache_write_price, period) }}
                    <template v-if="iv.cache_write_1h_price != null"
                      ><span class="font-sans font-normal text-muted"> (1h </span>{{ paidPerMillion(iv.cache_write_1h_price, period)
                      }}<span class="font-sans font-normal text-muted">)</span></template
                    >
                    <span class="ml-1 font-sans font-normal text-muted">{{ t('modelPlaza.table.cacheReadShort') }}</span>
              ...
              >
                <div>
                  <span class="mr-1 font-sans font-normal text-muted">{{ t('modelPlaza.table.cacheWrite') }}</span>
                  {{ paidPerMillion(m.pricing?.cache_write_price, period)
                  }}<template v-if="m.pricing?.cache_write_1h_price != null"
                    ><span class="font-sans font-normal text-muted"> (1h </span>{{ paidPerMillion(m.pricing.cache_write_1h_price, period)
                    }}<span class="font-sans font-normal text-muted">)</span></template
                  >
                </div>
                <div>
                  <span class="mr-1 font-sans font-normal text-muted">{{ t('modelPlaza.table.cacheRead') }}</span>
          ...
          <template v-else>
            <td colspan="3" class="pz-cell px-3 py-2.5 align-middle">
              ...
              >
                ...
                >
                  <span class="font-sans text-muted">{{ tierLabel(iv) }}</span>
                  {{ paidRequestPrice(m, iv.per_request_price)
                  }}<span class="font-sans text-muted">{{ perUnitSuffix(m) }}</span>
                </span>
              </div>
              <template v-else-if="m.pricing?.per_request_price != null">
                ...
                <span class="ml-1 text-xs text-muted">{{ perUnitSuffix(m) }}</span>
              </template>
              <span v-else class="text-muted">-</span>
            </td>
          </template>
          ...
          >
            <template v-if="officialIntervals(m).length">
              ...
              >
                <span class="mr-1 font-sans text-muted" :title="t('modelPlaza.table.tierHint')">{{ tierLabel(iv) }}</span>
          ...
          <td class="px-3 py-2.5 align-middle">
            <template v-if="hasTierCachePricing(officialIntervals(m))">
              ...
              >
                <template v-if="iv.cache_write_price != null || iv.cache_write_1h_price != null || iv.cache_read_price != null">
                  <span class="font-sans text-muted">{{ t('modelPlaza.table.cacheWriteShort') }}</span>
                  {{ official(iv.cache_write_price) }}
                  <template v-if="iv.cache_write_1h_price != null"
                    ><span class="font-sans text-muted"> (1h </span>{{ official(iv.cache_write_1h_price)
                    }}<span class="font-sans text-muted">)</span></template
                  >
                  <span class="ml-1 font-sans text-muted">{{ t('modelPlaza.table.cacheReadShort') }}</span>
            ...
            >
              <div>
                ...
                {{ official(m.official_pricing.cache_write_price)
                  ...
                  ><span class="font-sans text-muted"> (1h </span>{{ official(m.official_pricing.cache_write_1h_price)
                  }}<span class="font-sans text-muted">)</span></template
                >
              </div>
              <div>
          ...
          >
            ...
            <template v-else-if="hasCustomRate">
              <span class="mr-1 text-muted line-through">{{ rateMultiplier }}x</span>
