<template>
  <div class="relative inline-block">
    <span
      ref="triggerEl"
      :class="[
        'inline-flex cursor-help items-center gap-1 rounded-none border px-2 py-0.5 text-xs font-medium transition-colors',
        effectivePlatform
    ...
    <Teleport to="body">
      <div
        ...
        ref="popoverEl"
        role="tooltip"
        class="pointer-events-none fixed z-[99999] w-80 max-w-[min(22rem,calc(100vw-1rem))] rounded-none border bg-white text-xs shadow-outline dark:bg-dark-800"
        :class="[popoverBorderClass]"
        :style="popoverStyle"
      >
        <!-- Header：平台主题色背景，含模型名 + 平台徽章 -->
        <div
          class="flex items-center justify-between gap-2 rounded-none border-b px-3 py-2"
