<template>
  <section v-if="state?.eligible" class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600" data-testid="ollama-cloud-usage-settings">
    <div class="flex items-start justify-between gap-4">
      ...
      <span
        class="whitespace-nowrap rounded-none px-2 py-1 text-xs font-medium"
    ...
    <template v-else>
      <div v-if="!state.encryption_key_configured" class="rounded-none border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800/50 dark:bg-amber-900/20 dark:text-amber-200">
