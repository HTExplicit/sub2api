<template>
  <div v-for="entry in displayed" :key="entry.identity" class="mt-3">
    <label class="flex flex-col gap-1.5 text-sm font-medium">
      ...
      <input v-else-if="entry.field.kind === 'text'" type="text" class="input w-full" :placeholder="entry.field.placeholder" :value="values[entry.field.key] || ''" :disabled="disabled || !entry.available"
        :data-extension-field="entry.field.key" @input="update(entry.field.key, ($event.target as HTMLInputElement).value)" />
    ...
    <div v-if="entry.field.kind === 'textarea' || entry.field.kind === 'text'" class="mt-1 flex justify-between gap-3 text-xs text-muted">
    ...
    <p v-if="entry.field.kind !== 'select' && textLength(entry.field) > (entry.field.max_length || 0)" role="alert" class="text-sm text-red-600">{{ label(entry.field.limit_message || {}) || t('common.error') }}</p>
...
import type { PluginContribution, PluginFormField } from '@/api/admin/plugins'
...
import { contributionAdmission, createContributionAdmission } from './contributionAdmission'

const props = withDefaults(defineProps<{ name: string; values: Record<string, string>; context: Record<string, unknown>; account?: AccountSelectionIdentity | null; disabled?: boolean; contribution?: PluginContribution; createTarget?: { platform: string; accountType: string } }>(), { disabled: false })
...
const declared = computed(() => (props.contribution ? [props.contribution] : registry.items).filter(item => item.slot === props.name && item.permission === 'admin').flatMap(item => (item.fields || []).map(field => ({ field,
  available: props.createTarget ? createContributionAdmission(item, props.createTarget).allowed : contributionAdmission(item, { account: props.account }).allowed,
  identity: `${item.plugin_id}:${item.id}:${field.key}` }))))
...
const displayed = computed(() => declared.value.filter(entry => entry.field.kind !== 'select' || levels(entry.field).length > 0))
...
watch([declared, () => props.context, () => props.values], () => {
  ...
  emit('validity', declared.value.every(entry => (!props.values[entry.field.key] || entry.available) &&
    (entry.field.kind === 'select' || textLength(entry.field) <= (entry.field.max_length || 0))))
