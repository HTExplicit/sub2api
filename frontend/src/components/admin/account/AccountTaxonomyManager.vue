      ...
      <div class="overflow-hidden rounded-none border border-gray-200 dark:border-dark-700">
        ...
        <div v-for="item in visibleItems" :key="item.id" :data-test="`taxonomy-item-${item.id}`" class="flex min-h-12 items-center gap-3 border-b border-gray-100 px-3 py-2 last:border-b-0 dark:border-dark-700">
          ...
          <template v-else>
            ...
            <button type="button" data-test="taxonomy-delete" class="icon-button text-red-500 hover:text-red-600 disabled:opacity-50" :disabled="saving"
              :title="t('common.delete')" @click="requestDelete(item)">
