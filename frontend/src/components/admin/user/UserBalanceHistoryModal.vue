    ...
    <div v-if="user" class="space-y-4">
        ...
        <div class="flex items-center gap-3">
          ...
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              <span v-if="user.deleted_at" class="flex-shrink-0 inline-flex items-center rounded-none px-1 py-px text-[10px] font-medium leading-tight bg-rose-100 text-rose-600 ring-1 ring-inset ring-rose-200 dark:bg-rose-500/20 dark:text-rose-400 dark:ring-rose-500/30">
                {{ t('admin.usage.userDeletedBadge') }}
              </span>
              <span
                v-if="user.username"
                class="flex-shrink-0 rounded-none bg-primary-50 px-1.5 py-0.5 text-xs text-primary-600 dark:bg-primary-900/20 dark:text-primary-400"
              >
                {{ user.username }}
              </span>
            </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button
          v-if="!hideActions"
          @click="emit('deposit')"
          class="flex items-center gap-2 rounded-none border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-50 dark:border-dark-600 dark:bg-dark-800 dark:text-gray-300 dark:hover:bg-dark-700"
        >
      ...
      <div v-else class="max-h-[28rem] space-y-3 overflow-y-auto">
        <div
          ...
          :key="item.id"
          class="rounded-none border border-gray-200 bg-white p-4 dark:border-dark-600 dark:bg-dark-800"
        >
