...
function configuredOptions() {
  const values = [...new Set(props.pageSizeOptions.filter(value => Number.isInteger(value) && value >= 5 && value <= 1000))].sort((a, b) => a - b)
  return values.length ? values : [10, 20, 50, 100]
}
function normalizeTablePageSize(value: number) {
  const options = configuredOptions(), size = Number.isInteger(value) && value >= 5 ? value : 20
  return options.find(option => option >= size) || options[options.length - 1]!
}
