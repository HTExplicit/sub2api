...
type OpenAIForwardResult struct {
	...
	// UpstreamTerminalStatus is the HTTP-style status carried by the WS terminal
	// payload (response.error.status_code and friends), 0 when absent. It lets
	// observe-only telemetry bucket post-output response.failed turns as 429/5xx.
	UpstreamTerminalStatus int
