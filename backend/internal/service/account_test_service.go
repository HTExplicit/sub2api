...
func (s *AccountTestService) FetchOpenAIAccountModels(ctx context.Context, account *Account) ([]openai.Model, error) {
	return s.fetchOpenAIAccountModels(ctx, account, true)
}

// FetchOpenAIAccountCatalogModels keeps the upstream model IDs. The admin test
// picker applies the saved mapping itself: configured names stay testable even
// when the provider catalog omits their target, and wildcard mappings select
// request IDs from this list.
func (s *AccountTestService) FetchOpenAIAccountCatalogModels(ctx context.Context, account *Account) ([]openai.Model, error) {
	return s.fetchOpenAIAccountModels(ctx, account, false)
}
...
func (s *AccountTestService) testBedrockAccountConnection(c *gin.Context, ctx context.Context, account *Account, testModelID string) error {
	...
	state := connectionStreamState{protocol: "claude", stopReason: result.StopReason, emit: func(event TestEvent) { s.sendEvent(c, event) }}
...
func (s *AccountTestService) testOpenAIAccountConnection(c *gin.Context, account *Account, modelID string, prompt string, mode string) error {
	...
	if !agentIdentityTaskRecoveryWasTried(ctx) {
		s.sendEvent(c, TestEvent{Type: "test_start", Model: upstreamTestModelID})
	}
...
func (s *AccountTestService) testAntigravityAccountConnection(c *gin.Context, account *Account, modelID string) error {
	...
	s.sendEvent(c, TestEvent{Type: "test_complete", Success: true, OutputLimited: result.OutputLimited})
	return nil
}
...
func (s *AccountTestService) processGeminiStream(c *gin.Context, body io.Reader) error {
	return s.processConnectionStream(c, body, "gemini", nil)
}
