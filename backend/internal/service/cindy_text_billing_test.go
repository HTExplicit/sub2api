...
func TestCalculateCindyCatalogTextCostOpenAI272KBoundary(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.model, func(t *testing.T) {
			atBoundary := UsageTokens{InputTokens: 172000, CacheReadTokens: 100000, OutputTokens: 11}
			...
			require.Zero(t, base.CacheCreationCost)
			...
			require.Zero(t, long.CacheCreationCost)
...
func TestCalculateCindyCatalogTextCostUsesV4PriceAndRejectsUnknownModel(t *testing.T) {
	t.Parallel()
	billingService := NewBillingService(&config.Config{}, nil)
...
func TestCalculateCindyCatalogTextCostUsesExactPriorityTier(t *testing.T) {
	t.Parallel()
...
func TestCalculateCindyCatalogTextCostFailsClosedForMissingNeededPrice(t *testing.T) {
	...
	cost, err := calculateCindyCatalogTextCost(
		billing,
		"kimi-k3",
		UsageTokens{InputTokens: 100, CacheCreationTokens: 1},
		1,
		"",
		false,
	)
