...
func TestOpenAIWSHTTPBridgeSessionIsolationAcrossSameSessionHash(t *testing.T) {
	...
	newContext := func(r *http.Request) *gin.Context {
		...
		c.Request.Header.Set(openAIWSTurnStateHeader, "untrusted-client-turn-state")
	...
	stateStore.BindSessionTurnState(groupID, seedHash, account.ID, "sentinel-turn-state", time.Hour)
	...
	gotState, ok := stateStore.GetSessionTurnState(groupID, seedHash, account.ID)
	require.True(t, ok)
	...
	originSeed := openAICodexTurnStateSeed(newContext(httptest.NewRequest(http.MethodGet, "/v1/responses", nil)))
