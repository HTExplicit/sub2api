...
func TestMigration236PreservesCurrentAllowlist(t *testing.T) {
	tx := testTx(t)
	ctx := context.Background()
	const configuration = `{"enabled":true,"models":["Case/Model","vendor-*"],"opaque":"preserve"}`
	var groupID int64
	require.NoError(t, tx.QueryRowContext(ctx, `
INSERT INTO groups (name, platform, rate_multiplier, status, model_allowlist)
VALUES ('migration-236-current', 'anthropic', 1, 'active', $1::jsonb)
RETURNING id
`, configuration).Scan(&groupID))
	...
	for range 2 {
		...
		require.NoError(t, tx.QueryRowContext(ctx,
			"SELECT model_allowlist::text FROM groups WHERE id = $1", groupID).Scan(&allowlist))
		require.JSONEq(t, configuration, allowlist)
		requireModelAllowlistColumnShape(ctx, t, tx)
	}
}
