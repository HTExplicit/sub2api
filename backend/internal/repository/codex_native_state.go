...
func (r *nativeCodexRepository) CompareSwapExtensionState(ctx context.Context, plugin string, req extensionv1.StateRequest) (extensionv1.StateResult, error) {
	...
	if req.Projection != nil {
		projectionData, err = lockNativeCodexAccountProjection(ctx, tx, plugin, req.Projection)
		if err != nil {
			return out, err
		}
	}
	...
	if req.Projection != nil {
		...
		if _, err = tx.ExecContext(ctx, `UPDATE accounts SET extra=jsonb_set(COALESCE(NULLIF(extra,'null'::jsonb),'{}'::jsonb),ARRAY[$2],$3::jsonb,true),updated_at=NOW() WHERE id=$1`, req.Projection.AccountID, service.NativeCodexAccountProjectionKey, raw); err != nil {
			return out, err
		}
...
func lockNativeCodexAccountProjection(ctx context.Context, tx *sql.Tx, plugin string, projection *extensionv1.AccountProjection) (map[string]any, error) {
	...
	values, _ := a.Extra[service.NativeCodexAccountProjectionKey].(map[string]any)
	if values == nil {
		values = make(map[string]any)
	}
	current := service.NativeCodexAccountProjection{Identity: projection.Identity, Scheduling: make(map[string]extensionv1.SchedulingConstraint)}
	if raw, err := json.Marshal(values[plugin]); err == nil {
		var previous service.NativeCodexAccountProjection
...
func (r *nativeCodexRepository) DueExtensionStates(ctx context.Context, plugin string, req extensionv1.DueStateRequest) ([]extensionv1.DueState, error) {
...
func (r *nativeCodexRepository) AcquireExtensionLease(ctx context.Context, plugin string, req extensionv1.LeaseRequest) (extensionv1.LeaseResult, error) {
...
func (r *nativeCodexRepository) ReleaseExtensionLease(ctx context.Context, plugin string, req extensionv1.LeaseRequest) (extensionv1.LeaseResult, error) {
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
