...
func (r *accountRepository) ObserveQuotaEstimate(ctx context.Context, point service.QuotaEstimateObservation) error {
	_, err := r.sql.ExecContext(ctx, `INSERT INTO account_quota_estimates(account_id,identity_hash,period_key,resets_at,baseline_at,baseline_utilization,baseline_cost,observed_at,utilization,account_cost)
		...
		baseline_at=CASE WHEN EXCLUDED.utilization<account_quota_estimates.utilization OR (EXCLUDED.utilization>account_quota_estimates.utilization AND EXCLUDED.account_cost<=account_quota_estimates.account_cost) THEN EXCLUDED.observed_at ELSE account_quota_estimates.baseline_at END,
		baseline_utilization=CASE WHEN EXCLUDED.utilization<account_quota_estimates.utilization OR (EXCLUDED.utilization>account_quota_estimates.utilization AND EXCLUDED.account_cost<=account_quota_estimates.account_cost) THEN EXCLUDED.utilization ELSE account_quota_estimates.baseline_utilization END,
		baseline_cost=CASE WHEN EXCLUDED.utilization<account_quota_estimates.utilization OR (EXCLUDED.utilization>account_quota_estimates.utilization AND EXCLUDED.account_cost<=account_quota_estimates.account_cost) THEN EXCLUDED.account_cost ELSE account_quota_estimates.baseline_cost END,
