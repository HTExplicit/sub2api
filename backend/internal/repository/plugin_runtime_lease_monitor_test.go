...
func TestRuntimeLeaseNormalReleaseCancelsProbeWithoutConcurrentClose(t *testing.T) {
	session := &runtimeLeaseMonitorSession{entered: make(chan struct{}), block: true}
	lease := newPluginRuntimeSessionLease(session, time.Millisecond, time.Second)
	select {
...
func TestRuntimeLeaseLossListenerCanReleaseWithoutDeadlock(t *testing.T) {
...
func TestRuntimeLeaseProbeTimeoutReportsLossWithoutRetry(t *testing.T) {
	session := &runtimeLeaseMonitorSession{entered: make(chan struct{}), block: true}
	lease := newPluginRuntimeSessionLease(session, time.Millisecond, 5*time.Millisecond)
	select {
	case <-lease.Done():
	case <-time.After(time.Second):
