...
具名调用的 `account_id` 由宿主从实际授权目标写入；权限与灰度在缓存和执行前检查。账号任务元数据中的 `plugin_id`、`plugin_generation` 也由宿主签定归属；创建事务检查当前启停与代次。失败重试保留原插件，显式重新校验当前版本与账号范围，不能通过重试扩大已收窄的绑定。旧代次任务不能在新进程上自动继续。
