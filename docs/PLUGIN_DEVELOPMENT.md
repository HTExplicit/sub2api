...
当前分支仍在验证上述生命周期：租约会话异常失联后的在途 IO、跨插件最后读取后的并发写入，以及部分初始化竞争尚有专项未决，不能把正常会话排空用例当作这些异常路径已通过。初始化与独立更新已互斥持久所有权，已固定或正在更新的包不会被迟到的初始化 Prepare/Complete 覆盖；具体剩余项见实施状态。
