...
import { ACCOUNT_VIEW_REFRESH_EVENT, BaseDialog, emitHostEvent, openHostJob, usePersistentDraft } from '@sub2api/plugin-ui'
...
const { t } = useI18n()
const draft = usePersistentDraft('account-cleanup-draft')
...
async function submit() {
  ...
  try {
    ...
    await emitHostEvent(ACCOUNT_VIEW_REFRESH_EVENT)
    await openHostJob(job.id)
  } catch (value) {
    if (disposed || current !== revision) return
...
function openResult() { if (resultID.value) void openHostJob(resultID.value) }
