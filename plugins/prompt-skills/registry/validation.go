...
// Exported policy entrypoint for independent contract fixtures.
func ValidateMarkdownClosure(name, content string, files map[string][]byte) error {
	return validateRemoteSkillMarkdownClosure(name, content, files)
}
