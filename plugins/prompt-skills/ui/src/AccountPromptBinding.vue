...
import { emitHostEvent, extractApiErrorCode, extractApiErrorMessage } from '@sub2api/plugin-ui'
import { rulesAPI, type PromptAccountBinding, type PromptBinding, type PromptBindingResult, type PromptRuleState } from './rules-api'
...
async function save() {
  ...
  try {
    ...
    if (disposed || current !== generation) return
    accounts.value = rows
    if (next.some(item => item.applied)) await emitHostEvent('changed', { account_ids: next.filter(item => item.applied).map(item => item.account_id) })
  } catch (value) { if (!disposed && current === generation) fail(value) } finally { if (!disposed && current === generation) busy.value = false }
}
