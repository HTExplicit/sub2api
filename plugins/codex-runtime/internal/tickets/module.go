...
func NewModule() *Module {
	...
	return &Module{config: Config{RoutingSchema: 2, FailClosed: true, Models: []string{"gpt-6-astra", "gpt-6-sol", "gpt-6-luna"}, RequestZstd: true}, epoch: ctx, cancel: cancel, slots: make(chan struct{}, 5), prepareProxy: prepareProxy}
}
...
func (m *Module) ApplyConfig(ctx context.Context, raw json.RawMessage) error {
	...
	m.config = cfg
	if m.host != nil {
		go m.migrateRoutingStates(m.epoch, m.host, cfg)
