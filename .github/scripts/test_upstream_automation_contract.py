...
class UpstreamAutomationContractTest(unittest.TestCase):
    ...
    def test_publisher_reuses_pr_validation_without_duplicate_business_tests(self):
        ...
        self.assertIn('Build release image without publication', self.publisher)
        self.assertIn('push: false', self.publisher)
        self.assertIn('load: true', self.publisher)
        self.assertIn('docker push "$IMAGE:$VERSION"', self.publisher)
        self.assertLess(self.publisher.index('Build release image without publication'),
                        self.publisher.index('docker push "$IMAGE:$VERSION"'))
